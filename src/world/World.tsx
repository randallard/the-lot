...
import { fistBumpMove, handFor, type Availability } from "../dance/contact-move";

/** One instance, so the driver's constraint-id cache is not invalidated every render. */
const FIST_BUMP = fistBumpMove();
...
interface WorldProps {
  ...
  /**
   * Written every frame with whether the bump is currently on offer, for the wheel.
   *
   * The predicate needs yaw, which only the rigs have — so it is computed in here and
   * published outward across the ref boundary rather than recomputed on the DOM side.
   */
  bumpAvailability?: React.RefObject<Availability>;
  onNpcWalkAway?: () => void;
  onNpcApproach?: () => void;
...
export function World({ onPart1Pickup, onPart2Pickup, part1CutsceneDone, inputDir, rushMode, rushTarget, trinketTracker, showNpc, npcRelaxing, onNpcClick, npcWheelHandlers, bumpRequest, bumpAvailability, onNpcWalkAway, onNpcApproach, cameraOffset, cameraLookAtOffset, hidePlayer, npcScreenPos, playerScreenPos, showGameNpcs, onMycoClick, onEmberClick, onSproutClick, mycoScreenPos, emberScreenPos, sproutScreenPos, mycoAsleep, emberAsleep, sproutAsleep, findTargetNpcId, npcTalking, partsCollected, initialPlayerPos, playerWorldPos, bodyShapes, playerAnimController }: WorldProps) {
  // While the bump owns the arms, both characters draw the hand shape the move is solved
  // on — otherwise a closed-fist bump is measured at the fist's radius and drawn at the
  // open hand's, and the two fists overlap by the difference.
  const [bumping, setBumping] = useState(false);
  const bumpMove = FIST_BUMP;
  const bumpHands = {
    player: handFor(bumpMove.constraints[0], "A"),
    npc: handFor(bumpMove.constraints[0], "B"),
  };
  ...
  return (
    <>
      ...
      {/* `playerArm`/`npcArm` below are the anatomically **right** arms, which
          ...
          has to ask for the group it actually means, with the matching `drivenKey`. */}
      {showNpc && bumpRequest && (
        <FistBumpDriver
          request={bumpRequest}
          playerRig={playerRig}
